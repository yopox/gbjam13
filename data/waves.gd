...
func follow(n: int, delay: float, spawn: Spawn) -> Array:
	var s: Array = []
	for i in range(n):
...
func squad(base_movement: Movement, enemies: Array[Enemy.Types], dt: Array[float], dp: Array[Vector2]) -> Array:
...
func circle(enemies: Array[Enemy.Types], starting_pos: Vector2, r: float, a_speed: float, a_offset: float = 0.0) -> Array:
	var n: int = len(enemies)
	var spawns: Array = []
	for i in range(n):
...
func gen_wave(type: Types, difficulty: int) -> Array:
	match type:
		...
		Types.ORBITING:
			var spawns: Array = [Spawn.new(0, gen_enemy(difficulty), MovLinear.new(Util.right(16, 0.5), 0))]
